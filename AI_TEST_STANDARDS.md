...
**⚠️ AVOID SYSTEM VARIABLE CONFLICTS:**
* **NEVER use generic names like `USERNAME` or `PASSWORD`** - these conflict with system environment variables (especially on Windows)
* **ALWAYS use prefixed names** like `TEST_USERNAME`, `APP_USERNAME`, `AUTH_USERNAME`
* Common system variables to avoid: `USERNAME`, `PASSWORD`, `PATH`, `TEMP`, `HOME`, `USER`
* Use descriptive prefixes: `TEST_`, `APP_`, `AUTH_`, `DB_`, etc.
...
# Authentication (ALWAYS use prefixed names!)
...
# Authentication (use prefixed names to avoid system conflicts)
TEST_USERNAME=
TEST_PASSWORD=
API_KEY=
